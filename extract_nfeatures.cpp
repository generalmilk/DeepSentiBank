...
int feature_extraction_pipeline(int argc, char** argv) {
  ...
  for (size_t i = 0; i < num_features; ++i) {
    ...
    //                                           &db);
    CHECK(1) << "Failed to open leveldb " << leveldb_names[i];
  ...
  vector<int> image_indices(num_features, 0);
   vector<std::ofstream*> write_bins;
  for (size_t i = 0; i < num_features; ++i) {
   write_bins.push_back(new std::ofstream((leveldb_names[i]+".dat").c_str(),std::ios::out|std::ios::binary));    
		
	if (!write_bins[i]->is_open())
	{
		printf("%s\n",(leveldb_names[i]+".dat").c_str());
	}
  }
